...
- **Built-in streaming apps** — always in the Add-app list:
  - *YouTube TV* — youtube.com/tv with the PS4 Leanback user agent (4K/60 + audio dubs), remote-first input. Back steps out of a video to browsing, Back again at home returns to the launcher; Home always returns to the launcher
  ...
  - *Spotify* — same Edge app-window + extension pattern as Netflix (web player, real Widevine for full-quality playback, Back walks the UI and exits to the launcher at the library root, Home always exits). Dedicated persisted-login profile. A remote-friendly player, not a desktop web page. While music plays, a now-playing widget appears in the launcher's top bar (album art + title + artist, plus a progress bar when available)
  ...
  - **D-pad navigation inside both apps** — the bundled extensions add a geometric spatial navigator: arrow keys move focus to the nearest element in the pressed direction anywhere on the page (with a bold white focus ring), so a plain remote works even on areas the site never made arrow-friendly. Land on any search/login box and a TV-sized **on-screen keyboard** pops up: D-pad to letters, Enter to press, Backspace to edit, Done to type-submit into the site. Printable keys from a physical remote still reach the field directly.
